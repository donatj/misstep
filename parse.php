...
//		}
		...
		switch( $bodyResult[$j]['colType'] ) {
			...
			case 'datetime':
				...
				break;
			default:
				throw new \Exception('Unknown type: ' . $bodyResult[$j]['colType']);
		}
		...
		if( $bodyResult[$j]['signed'] ) {
			if( $col instanceof Columns\Interfaces\SignedInterface ) {
				...
				throw new \Exception('type ' . $col->getTypeName() . ' cannot be signed');
			}
		}
		...
		if( $bodyResult[$j]['signal'] == '!' ) {
			if( !empty($foreignKeys['parents'][$col->getName()]) ) {
				throw new Exception("foreign key remote {$col->getName()} already defined.");
			}
...
function checkForParseErrors( $regex, $data ) {
	...
	foreach( $split as $i ) {
		if( trim($i) ) {
			throw new Exception('parse error on "' . var_export(trim($i) . '"', true));
		}
	}
}
